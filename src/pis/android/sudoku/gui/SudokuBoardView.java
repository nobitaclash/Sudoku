...
public class SudokuBoardView extends View {
	...
	public SudokuBoardView(Context context, AttributeSet attrs/* , int defStyle */) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.SudokuBoardView/* , defStyle, 0 */);

		setLineColor(a.getColor(R.styleable.SudokuBoardView_lineColor,
				Color.BLACK));
		...
		setBackgroundColor(a.getColor(
				R.styleable.SudokuBoardView_backgroundColor,
				getColor(context, R.color.bg_color))); //
		setBackgroundColorSecondary(a.getColor(
				R.styleable.SudokuBoardView_backgroundColorSecondary, NO_COLOR));
		setBackgroundColorReadOnly(a.getColor(
				R.styleable.SudokuBoardView_backgroundColorReadOnly,
				getColor(context, R.color.background_color_readOnly)));
		setBackgroundColorTouched(a.getColor(
				R.styleable.SudokuBoardView_backgroundColorTouched,
				getColor(context, R.color.touch_color)));//
		setBackgroundColorSelected(a.getColor(
				R.styleable.SudokuBoardView_backgroundColorSelected,
				getColor(context, R.color.select_color)));//
	...
	public void setCells(CellCollection cells) {
		...
		if (mCells != null) {
			if (!mReadonly) {
				mSelectedCell = mCells.getCell(0, 0); // first cell will be
														// selected by default
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mCells != null) {
			...
			if (!mReadonly && mSelectedCell != null) {
				cellLeft = Math.round(mSelectedCell.getColumnIndex()
						* mCellWidth)
						+ paddingLeft;
			...
			if (mHighlightTouchedCell && mTouchedCell != null) {
				cellLeft = Math.round(mTouchedCell.getColumnIndex()
						* mCellWidth)
						+ paddingLeft;
	...
	private boolean moveCellSelectionTo(int row, int col) {
		if (col >= 0 && col < CellCollection.SUDOKU_SIZE && row >= 0
				&& row < CellCollection.SUDOKU_SIZE) {
