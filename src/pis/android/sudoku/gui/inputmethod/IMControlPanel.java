...
import pis.android.sudoku.gui.SudokuBoardView.OnCellSelectedListener;
import pis.android.sudoku.gui.SudokuBoardView.OnCellTappedListener;
...
public class IMControlPanel extends LinearLayout {
	...
	/**
	 * Activates first enabled input method. If such method does not exists,
	 * nothing happens.
	 */
	public void activateFirstInputMethod() {
		ensureInputMethods();
		if (mActiveMethodIndex == -1
				|| !mInputMethods.get(mActiveMethodIndex).isEnabled()) {
	...
	/**
	 * Activates given input method (see INPUT_METHOD_* constants). If the given
	 * method is not enabled, activates first available method after this
	 * method.
	 * 
	 * @param methodID
	...
	public void activateInputMethod(int methodID) {
		...
		for (int i = 0; i < mInputMethods.size(); i++) {
			...
			if (im.isInputMethodViewCreated()) {
				im.getInputMethodView().setVisibility(
						i == id ? View.VISIBLE : View.GONE);
			}
		}
		...
		if (mActiveMethodIndex != -1) {
			...
			if (mHintsQueue != null) {
				mHintsQueue.showOneTimeHint(activeMethod.getInputMethodName(),
						activeMethod.getNameResID(),
						activeMethod.getHelpResID());
			}
		}
	}
	...
	public void activateNextInputMethod() {
		...
		if (id >= mInputMethods.size()) {
			if (mHintsQueue != null) {
				mHintsQueue.showOneTimeHint("thatIsAll", R.string.that_is_all,
						R.string.im_disable_modes_hint);
			}
			id = 0;
		}
	...
	public void showHelpForActiveMethod() {
		...
		if (mActiveMethodIndex != -1) {
			...
			mHintsQueue.showHint(activeMethod.getNameResID(),
					activeMethod.getHelpResID());
		}
	}
	...
	/**
	 * This should be called when activity is paused (so Input Methods can do
	 * some cleanup, for example properly dismiss dialogs because of
	...
	private void ensureInputMethods() {
		if (mInputMethods.size() == 0) {
			throw new IllegalStateException(
					"Input methods are not created yet. Call initialize() first.");
		}
	...
	private void ensureControlPanel(int methodID) {
		...
		if (!im.isInputMethodViewCreated()) {
			...
			// (Button)controlPanel.findViewById(R.id.switch_input_mode);
			// switchModeButton.setOnClickListener(mSwitchModeListener);
			this.addView(controlPanel, LayoutParams.FILL_PARENT,
