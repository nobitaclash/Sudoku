...
/**
 * Collection of sudoku cells. This class in fact represents one sudoku board
 * (9x9).
 * 
 * @author romario
 * 
...
public class CellCollection {
	...
	// TODO: An array of ints is a much better than an array of Integers, but
	// this also generalizes to the fact that two parallel arrays of ints are
	// also a lot more efficient than an array of (int,int) objects
	// Cell's data.
	private Cell[][] mCells;

	// Helper arrays, contains references to the groups of cells, which should
	// contain unique
	// numbers.
	...
	public static CellCollection createDebugGame() {
		CellCollection debugGame = new CellCollection(new Cell[][] {
				{ new Cell(), new Cell(), new Cell(), new Cell(4), new Cell(5),
						new Cell(6), new Cell(7), new Cell(8), new Cell(9), },
				{ new Cell(), new Cell(), new Cell(), new Cell(7), new Cell(8),
						new Cell(9), new Cell(1), new Cell(2), new Cell(3), },
				{ new Cell(), new Cell(), new Cell(), new Cell(1), new Cell(2),
						new Cell(3), new Cell(4), new Cell(5), new Cell(6), },
				{ new Cell(2), new Cell(3), new Cell(4), new Cell(),
						new Cell(), new Cell(), new Cell(8), new Cell(9),
						new Cell(1), },
				{ new Cell(5), new Cell(6), new Cell(7), new Cell(),
						new Cell(), new Cell(), new Cell(2), new Cell(3),
						new Cell(4), },
				{ new Cell(8), new Cell(9), new Cell(1), new Cell(),
						new Cell(), new Cell(), new Cell(5), new Cell(6),
						new Cell(7), },
				{ new Cell(3), new Cell(4), new Cell(5), new Cell(6),
						new Cell(7), new Cell(8), new Cell(9), new Cell(1),
						new Cell(2), },
				{ new Cell(6), new Cell(7), new Cell(8), new Cell(9),
	...
	/**
	 * Validates numbers in collection according to the sudoku rules. Cells with
	 * invalid values are marked - you can use getInvalid method of cell to find
	 * out whether cell contains valid value.
	 * 
	...
	/**
	 * Initializes collection, initialization has two steps: 1) Groups of cells
	 * which must contain unique numbers are created. 2) Row and column index
	 * for each cell is set.
	 */
	private void initCollection() {
		...
		for (int r = 0; r < SUDOKU_SIZE; r++) {
			for (int c = 0; c < SUDOKU_SIZE; c++) {
				...
				cell.initCollection(this, r, c, mSectors[((c / 3) * 3)
						+ (r / 3)], mRows[c], mColumns[r]);
			}
		}
	}
	...
	public static CellCollection deserialize(String data) {
		...
		if (lines.length == 0) {
			throw new IllegalArgumentException(
					"Cannot deserialize Sudoku, data corrupted.");
		}
	...
	/**
	 * Creates collection instance from given string. String is expected to be
	 * in format "00002343243202...", where each number represents cell value,
	 * no other information can be set using this method.
	 * 
	 * @param data
	 * @return
	 */
	...
	/**
	 * Writes collection to given StringBuilder. You can later recreate the
	 * object instance by calling {@link #deserialize(String)} method.
	 * 
	 * @return
	 */
	...
	public String toString() {
		...
		for (int r = 0; r < CellCollection.SUDOKU_SIZE; r++) {
			for (int c = 0; c < CellCollection.SUDOKU_SIZE; c++) {
				buffer.append(mCells[r][c].getValue());
			}
		}
